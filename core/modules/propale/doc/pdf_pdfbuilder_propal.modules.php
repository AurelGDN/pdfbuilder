...
/**
 * \file       pdfbuilder/core/modules/propale/pdf_pdfbuilder_propal.class.php
 * \ingroup    pdfbuilder
 * \brief      Modèle PDF pour les devis (propositions commerciales) - PDF-builder
 */
...
/**
 * Classe de génération PDF pour les devis avec PDF-builder
 */
class pdf_pdfbuilder_propal extends ModelePDFPropales
{
    public $name = 'pdfbuilder_propal';
    public $description = 'Modèle PDF-builder pour les devis';
    ...
    public function __construct($db)
    {
        ...
        $this->db = $db;
        $this->theme = PdfBuilderTheme::getActive($db, 'propal');

        $langs->loadLangs(array('main', 'bills', 'products', 'dict', 'companies', 'propal', 'pdfbuilder@pdfbuilder'));
    ...
    public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
    {
        ...
        $outputlangs->loadLangs(array('main', 'dict', 'companies', 'propal', 'products', 'pdfbuilder@pdfbuilder'));

        $this->theme = PdfBuilderTheme::getActive($this->db, 'propal');
        $nblines = count($object->lines);

        // Répertoire de sortie
        if ($conf->propal->multidir_output[$object->entity]) {
        ...
        $pdf->SetSubject($outputlangs->transnoentities('PdfPropalTitle'));
    ...
    private function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
    {
        ...
        $pdf->MultiCell(90, 7, $outputlangs->transnoentities('PdfPropalTitle'), 0, 'R', false);

        $posy_right = $posy + 8;

        // Référence
        $pdf->SetXY($this->page_largeur - $this->marge_droite - 90, $posy_right);
        $pdf->SetFont($theme->font_family ?: 'DejaVuSans', $theme->font_style_ref ?: 'B', (int) ($theme->font_size_ref ?: 10));
        ...
        if (!empty($theme->show_customer_code) && !empty($object->thirdparty->code_client)) {
            $pdf->SetXY($this->page_largeur - $this->marge_droite - 90, $posy_right);
            $pdf->SetFont($theme->font_family ?: 'DejaVuSans', $theme->font_style_ref, (int) ($theme->font_size_ref ?: 8));
            $pdf->SetTextColor($colorFont['r'], $colorFont['g'], $colorFont['b']);
        ...
        // Date + validité
        $pdf->SetXY($this->page_largeur - $this->marge_droite - 90, $posy_right + 1);
        $pdf->SetFont($theme->font_family ?: 'DejaVuSans', '', 8);
        $pdf->SetTextColor($colorFont['r'], $colorFont['g'], $colorFont['b']);
        $pdf->MultiCell(90, 5, $outputlangs->transnoentities('Date').': '.dol_print_date($object->date, 'day', false, $outputlangs, true), 0, 'R', false);
        $posy_right += 6;

        if ($object->fin_validite) {
            $pdf->SetXY($this->page_largeur - $this->marge_droite - 90, $posy_right);
            $pdf->MultiCell(90, 5, $outputlangs->transnoentities('DateEndPropal').': '.dol_print_date($object->fin_validite, 'day', false, $outputlangs, true), 0, 'R', false);
            $posy_right += 5;
        }
        ...
        if ($showaddress) {
            ...
            if ($mysoc->siret) $senderLines .= "\nSIRET : ".$mysoc->siret;
            if ($mysoc->tva_intra) $senderLines .= "\nTVA : ".$mysoc->tva_intra;
            ...
            if (is_object($thirdparty)) {
                ...
                if ($thirdparty->address) $recipLines .= $thirdparty->address."\n";
                $recipLines .= $thirdparty->zip.' '.$thirdparty->town;
                if ($thirdparty->country) $recipLines .= "\n".$thirdparty->country;
                if ($thirdparty->tva_intra) $recipLines .= "\nTVA : ".$thirdparty->tva_intra;
    ...
    private function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop = 0, $hidebottom = 0, $currency = '')
    {
        ...
        if ($hidetop == 0) {
            ...
            $pdf->Rect($curX, $tab_top, $this->page_width, 6, 'F');
            ...
            if (!empty($theme->show_line_numbers)) {
                ...
                $pdf->SetXY($colX, $tab_top + 1); $pdf->Cell($w, 4, '#', 0, 0, 'C', false);
                $colX += $w;
            }
            if (!empty($theme->show_ref_column)) {
                $w = (float) ($theme->col_width_ref ?: 22);
                $pdf->SetXY($colX, $tab_top + 1); $pdf->Cell($w, 4, $outputlangs->transnoentities('Ref'), 0, 0, 'L', false);
                $colX += $w;
            }

            $descWidth = $this->_getDescWidth();
            $pdf->SetXY($colX, $tab_top + 1); $pdf->Cell($descWidth, 4, $outputlangs->transnoentities('Description'), 0, 0, 'L', false);
            $colX += $descWidth;

            if (empty($theme->hide_vat_column)) {
                $w = (float) ($theme->col_width_vat ?: 13);
                $pdf->SetXY($colX, $tab_top + 1); $pdf->Cell($w, 4, $outputlangs->transnoentities('VAT'), 0, 0, 'C', false);
                $colX += $w;
            }
            if (empty($theme->hide_puht)) {
                $w = (float) ($theme->col_width_puht ?: 19);
                $pdf->SetXY($colX, $tab_top + 1); $pdf->Cell($w, 4, $outputlangs->transnoentities('PriceUHT'), 0, 0, 'R', false);
                $colX += $w;
            }
            if (empty($theme->hide_qty)) {
                $w = (float) ($theme->col_width_qty ?: 13);
                $pdf->SetXY($colX, $tab_top + 1); $pdf->Cell($w, 4, $outputlangs->transnoentities('Qty'), 0, 0, 'C', false);
                $colX += $w;
            }
            if (empty($theme->hide_discount)) {
                $w = (float) ($theme->col_width_discount ?: 12);
                $pdf->SetXY($colX, $tab_top + 1); $pdf->Cell($w, 4, $outputlangs->transnoentities('ReductionShort'), 0, 0, 'C', false);
                $colX += $w;
            }

            $w = (float) ($theme->col_width_total ?: 22);
            $pdf->SetXY($colX, $tab_top + 1);
    ...
    private function _getDescWidth()
    {
        ...
        if (!empty($theme->show_pictures))     $width -= (float) ($theme->col_width_img ?: 20);
    ...
    private function _outputLine(&$pdf, $object, $i, $curY, $outputlangs, $heightff, $heightft, $tab_top_np)
    {
        ...
        if (!empty($theme->show_line_numbers)) {
            $w = (float) ($theme->col_width_number ?: 8);
            $pdf->SetXY($colX, $curY);
            $pdf->SetFont($theme->font_family ?: 'DejaVuSans', $theme->font_style_desc, (int) ($theme->font_size_desc ?: 7));
        ...
        if (empty($theme->hide_vat_column)) {
            $w = (float) ($theme->col_width_vat ?: 13);
            $pdf->SetXY($colX, $curY);
            $pdf->Cell($w, $cellH, (!empty($line->tva_tx) ? price($line->tva_tx, 0, $outputlangs).'%' : ''), 0, 0, 'C', false);
            $colX += $w;
        }
        if (empty($theme->hide_puht)) {
            $w = (float) ($theme->col_width_puht ?: 19);
    ...
    private function _pagefoot(&$pdf, $object, $outputlangs, $hidefreetext = 0)
    {
        ...
        if (!empty($theme->show_bon_accord) || !empty($theme->show_signature)) {
            $sigY = $curY + 12;
            if (!empty($theme->show_bon_accord)) {
                $pdf->SetFont($theme->font_family ?: 'DejaVuSans', 'B', 8);
                $pdf->SetXY($this->marge_gauche, $sigY);
                $pdf->MultiCell(80, 5, $outputlangs->transnoentities('BonPourAccord'), 0, 'L', false);
                $sigY += 6;
            }
            if (!empty($theme->show_signature)) {
                $pdf->SetFont($theme->font_family ?: 'DejaVuSans', '', 7);
                $pdf->SetXY($this->marge_gauche, $sigY);
                $pdf->MultiCell(80, 5, $outputlangs->transnoentities('Date').' : ___/___/______       '.$outputlangs->transnoentities('Signature').' :', 0, 'L', false);
                $pdf->Rect($this->marge_gauche, $sigY + 6, 80, 20);
            }
        }
        ...
        $footerParts = array();
        if ($mysoc->name) $footerParts[] = $mysoc->name;
        if ($mysoc->siret) $footerParts[] = 'SIRET : '.$mysoc->siret;
        if ($mysoc->tva_intra) $footerParts[] = 'TVA : '.$mysoc->tva_intra;
        if ($mysoc->capital) $footerParts[] = 'Capital : '.price($mysoc->capital, 0, $outputlangs).' '.$conf->currency;
