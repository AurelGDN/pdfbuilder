...
/**
 * \file       pdfbuilder/core/modules/commande/pdf_pdfbuilder_order.class.php
 * \ingroup    pdfbuilder
 * \brief      Modèle PDF pour les commandes clients - PDF-builder
 */
...
/**
 * Classe de génération PDF pour les commandes clients avec PDF-builder
 */
class pdf_pdfbuilder_order extends ModelePDFCommandes
{
    public $name = 'pdfbuilder_order';
    public $description = 'Modèle PDF-builder pour les commandes clients';
    ...
    public function __construct($db)
    {
        ...
        $this->db = $db;
        $this->theme = PdfBuilderTheme::getActive($db, 'order');
    ...
    public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
    {
        ...
        $this->theme = PdfBuilderTheme::getActive($this->db, 'order');
        $nblines = count($object->lines);

        if ($conf->commande->multidir_output[$object->entity]) {
    ...
    private function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
    {
        ...
        $pdf->MultiCell(90, 7, $outputlangs->transnoentities('PdfOrderTitle'), 0, 'R', false);
        ...
        if ($object->delivery_date) {
            $pdf->SetXY($this->page_largeur - $this->marge_droite - 90, $posy_right);
            $pdf->MultiCell(90, 5, $outputlangs->transnoentities('DateDeliveryPlanned').': '.dol_print_date($object->delivery_date, 'day', false, $outputlangs, true), 0, 'R', false);
            $posy_right += 5;
        }
    ...
    private function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop = 0, $hidebottom = 0, $currency = '')
    {
        ...
        if ($hidetop == 0) {
            ...
            if (!empty($theme->show_line_numbers)) {
                ...
                $pdf->SetXY($colX, $tab_top + 1); $pdf->Cell($w, 4, '#', 0, 0, 'C'); $colX += $w;
            }
            if (!empty($theme->show_ref_column)) {
                $w = (float) ($theme->col_width_ref ?: 22);
                $pdf->SetXY($colX, $tab_top + 1); $pdf->Cell($w, 4, $outputlangs->transnoentities('Ref'), 0, 0, 'L'); $colX += $w;
            }
    ...
    private function _outputLine(&$pdf, $object, $i, $curY, $outputlangs, $heightff, $heightft, $tab_top_np)
    {
        ...
        if (!empty($theme->show_line_numbers)) {
            $w = (float) ($theme->col_width_number ?: 8);
            $pdf->SetXY($colX, $curY); $pdf->SetFont($theme->font_family ?: 'DejaVuSans', $theme->font_style_desc, (int) ($theme->font_size_desc ?: 7)); $pdf->Cell($w, $cellH, ($i + 1), 0, 0, 'C');
            $pdf->SetFont($theme->font_family ?: 'DejaVuSans', $theme->font_style_desc, (int) ($theme->font_size_desc ?: 9)); $colX += $w;
        }
        if (!empty($theme->show_ref_column)) {
            $w = (float) ($theme->col_width_ref ?: 22);
            $pdf->SetXY($colX, $curY); $pdf->MultiCell($w, $cellH, $outputlangs->convToOutputCharset($line->product_ref ?: ''), 0, 'L'); $colX += $w;
        }
        ...
        if ($descLines && empty($theme->hide_desc_long)) {
            $pdf->SetXY($colX, $pdf->GetY()); $pdf->SetFont($theme->font_family ?: 'DejaVuSans', '', (int) ($theme->font_size ?: 9) - 1);
            $pdf->MultiCell($descWidth, $lineH - 1.5, $descLines, 0, 'L');
        }
        $colX += $descWidth;

        if (empty($theme->hide_vat_column)) { $w = (float) ($theme->col_width_vat ?: 13); $pdf->SetXY($colX, $curY); $pdf->Cell($w, $cellH, (!empty($line->tva_tx) ? price($line->tva_tx, 0, $outputlangs).'%' : ''), 0, 0, 'C'); $colX += $w; }
    ...
    private function _pagefoot(&$pdf, $object, $outputlangs, $hidefreetext = 0)
    {
        ...
        if ($hidefreetext == 0) {
            ...
            if ($noteText) {
                ...
                $pdf->MultiCell($this->page_width - $totalWidth - 5, (float) ($theme->freetext_height ?: 12), $noteText, 0, 'L');
            }
        }
        ...
        $fp = array();
        if ($mysoc->name) $fp[] = $mysoc->name;
        if ($mysoc->siret) $fp[] = 'SIRET : '.$mysoc->siret;
        if ($mysoc->tva_intra) $fp[] = 'TVA : '.$mysoc->tva_intra;
        if ($mysoc->capital) $fp[] = 'Capital : '.price($mysoc->capital, 0, $outputlangs).' '.$conf->currency;
