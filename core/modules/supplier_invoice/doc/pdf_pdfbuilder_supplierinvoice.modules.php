...
/**
 * \file       pdfbuilder/core/modules/supplier_invoice/pdf_pdfbuilder_supplierinvoice.class.php
 * \ingroup    pdfbuilder
 * \brief      Modèle PDF pour les factures fournisseurs - PDF-builder
 */
...
/**
 * Classe de génération PDF pour les factures fournisseurs avec PDF-builder
 */
class pdf_pdfbuilder_supplierinvoice extends ModelePDFSuppliersInvoices
{
    public $name = 'pdfbuilder_supplierinvoice';
    public $description = 'Modèle PDF-builder pour les factures fournisseurs';
    ...
    public function __construct($db)
    {
        ...
        $this->db = $db;
        $this->theme = PdfBuilderTheme::getActive($db, 'supplier_invoice');
    ...
    public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
    {
        ...
        $this->theme = PdfBuilderTheme::getActive($this->db, 'supplier_invoice');
        $nblines = count($object->lines);

        if ($conf->fournisseur->facture->multidir_output[$object->entity]) {
            $dir = $conf->fournisseur->facture->multidir_output[$object->entity];
        } elseif (isset($conf->supplier_invoice->multidir_output[$object->entity])) {
            $dir = $conf->supplier_invoice->multidir_output[$object->entity];
        } else {
        ...
        if (!preg_match('/specimen/i', $objectref)) {
            $subdir = dol_sanitizeFileName($object->ref_supplier);
            $dir .= '/'.get_exdir($object->id, 2, 0, 0, $object, 'invoice_supplier').$objectref;
        }
        ...
        $pdf->SetSubject($outputlangs->transnoentities('SupplierInvoice'));
    ...
    private function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
    {
        ...
        $pdf->MultiCell(90, 7, $outputlangs->transnoentities('SupplierInvoice'), 0, 'R', false);
        ...
        if ($object->ref_supplier) {
            ...
            $pdf->MultiCell(90, 5, $outputlangs->transnoentities('RefSupplier').': '.$outputlangs->convToOutputCharset($object->ref_supplier), 0, 'R', false);
            $nextY += 5;
        }
        ...
        if ($showaddress) {
            ...
            // Pour les fournisseurs, le fournisseur est l'émetteur et nous sommes le destinataire
            $addrLeft  = $this->marge_gauche;
            $addrRight = $this->marge_gauche + $blkWidth + 4;
            ...
            if (is_object($thirdparty)) {
                $supLines = $thirdparty->name."\n";
                if ($thirdparty->address) $supLines .= $thirdparty->address."\n";
                $supLines .= $thirdparty->zip.' '.$thirdparty->town;
                if ($thirdparty->tva_intra) $supLines .= "\nTVA : ".$thirdparty->tva_intra;
                $pdf->MultiCell($blkWidth - 4, 4, $outputlangs->convToOutputCharset($supLines), 0, 'L', false);
            }

            // Notre société (destinataire)
            ...
            $recipLines = $mysoc->name."\n".$mysoc->address."\n".$mysoc->zip.' '.$mysoc->town;
            if ($mysoc->siret) $recipLines .= "\nSIRET : ".$mysoc->siret;
